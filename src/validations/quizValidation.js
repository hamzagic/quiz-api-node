...
const quizInputValidation = [
  check('quizName').trim().isLength({ min: 6 }).escape().withMessage('A quiz name is required'),
  check('creator').trim().isLength({ min: 3}).escape().withMessage('Quiz creator is required'),
  check('numberOfQuestions').trim().isInt().withMessage('Total questions must be an integer'),
